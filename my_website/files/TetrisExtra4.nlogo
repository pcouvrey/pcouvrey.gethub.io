...
to new-piece
  ...
  set next-shape random 11
  if (show-next?) [ show-next-piece ]
  ask turtle 0 [ setxy -3 10 ]
  ask pieces
...
to setup-piece [s]  ;; Piece Procedure
  ...
  if (s = 7) [ setup-d  set color white ]
  if (s = 8) [ setup-c  set color 24 ]
  if (s = 9) [ setup-p  set color magenta ]
  if (s = 10) [ setup-n  set color lime ]
...
;;10
;;  23
to setup-d ;;Piece Procedure
  if (who = 1) [ set x -1 set y 0 ]
  if (who = 2) [ set x 1 set y -1 ]
  if (who = 3) [ set x 2 set y -1 ]
end

;; C-Block
;; 01
;;2  3
to setup-c ;;Piece Procedure
  if (who = 1) [ set x -1 set y -1 ]
  if (who = 2) [ set x 1 set y 0 ]
  if (who = 3) [ set x 2 set y -1 ]
end
